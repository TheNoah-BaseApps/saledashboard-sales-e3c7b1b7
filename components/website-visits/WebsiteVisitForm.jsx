...
export default function WebsiteVisitForm({ visit, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    ...
    page_visits: visit?.page_visits ? visit.page_visits.join(', ') : '',
    website_duration: visit?.website_duration || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        page_visits: formData.page_visits.split(',').map(p => p.trim()).filter(Boolean),
        number_of_visits: parseInt(formData.number_of_visits),
        website_duration: parseInt(formData.website_duration),
      };

      const url = visit ? `/api/website-visits/${visit.id}` : '/api/website-visits';
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to save website visit');
      }

      onSuccess?.();
    } catch (err) {
      console.error('Error saving website visit:', err);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, website_duration: e.target.value })}
          />
        </div>
      ...
      <div className="space-y-2">
        <Label htmlFor="page_visits">Pages Visited (comma-separated)</Label>
        <Textarea
          id="page_visits"
