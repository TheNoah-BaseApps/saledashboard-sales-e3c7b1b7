 ...
 *   get:
 *     summary: Get a single email interaction
...
export async function GET(request, { params }) {
  try {
    const { id } = params;
    const result = await query('SELECT * FROM email_interactions WHERE id = $1', [id]);
 ...
 * /api/email-interactions/{id}:
 *   put:
 *     summary: Update an email interaction
 *     tags: [Email Interactions]
 *     parameters:
 *       - in: path
 *         name: id
  ...
  try {
    ...
    const result = await query(
      `UPDATE email_interactions 
       SET email_ids = $1, email_domain = $2, subject = $3, message = $4, attachments = $5, 
           time = $6, date = $7, thread = $8, summary = $9, sentiment = $10, 
           sender_id = $11, receiver_id = $12, cc_id = $13, updated_at = NOW() 
       WHERE id = $14 
       RETURNING *`,
 ...
 * /api/email-interactions/{id}:
 *   delete:
 *     summary: Delete an email interaction
 *     tags: [Email Interactions]
 *     parameters:
 *       - in: path
 *         name: id
...
export async function DELETE(request, { params }) {
  try {
    const { id } = params;
    const result = await query('DELETE FROM email_interactions WHERE id = $1 RETURNING *', [id]);
