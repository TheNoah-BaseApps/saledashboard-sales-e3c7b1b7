 ...
 *   get:
 *     summary: Get a single newsletter blog entry
...
export async function GET(request, { params }) {
  try {
    const { id } = params;
    const result = await query('SELECT * FROM newsletter_blogs WHERE id = $1', [id]);
 ...
 * /api/newsletter-blogs/{id}:
 *   put:
 *     summary: Update a newsletter blog entry
 *     tags: [Newsletter Blogs]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 * /api/newsletter-blogs/{id}:
 *   delete:
 *     summary: Delete a newsletter blog entry
 *     tags: [Newsletter Blogs]
 *     parameters:
 *       - in: path
 *         name: id
...
export async function DELETE(request, { params }) {
  try {
    const { id } = params;
    const result = await query('DELETE FROM newsletter_blogs WHERE id = $1 RETURNING *', [id]);
