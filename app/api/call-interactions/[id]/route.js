 ...
 *   get:
 *     summary: Get a single call interaction
...
export async function GET(request, { params }) {
  try {
    const { id } = params;
    const result = await query('SELECT * FROM call_interactions WHERE id = $1', [id]);
 ...
 * /api/call-interactions/{id}:
 *   put:
 *     summary: Update a call interaction
 *     tags: [Call Interactions]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 * /api/call-interactions/{id}:
 *   delete:
 *     summary: Delete a call interaction
 *     tags: [Call Interactions]
 *     parameters:
 *       - in: path
 *         name: id
...
export async function DELETE(request, { params }) {
  try {
    const { id } = params;
    const result = await query('DELETE FROM call_interactions WHERE id = $1 RETURNING *', [id]);
